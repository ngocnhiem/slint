...
There are a few other repository in the organization, they are either
 ...
 - Website repository
 - Other repository for code that needs to be kept private as it is part of a non-open source product

But all the code of the Slint product is in the mono-repository.
...
The administrators of the slint-ui Github organization are the founder of SixtyFPS GmbH.
All employee of SixtyFPS GmbH are members of the organization.
Some other people may also be invited to be member of the Github organization if they have contributed to the project.

## Code changes

The development happens in the `master` branch.
Developer can create a branch `<developer-name>/<feature-name>` in the slint-ui/slint repository to make a PR, although some developer prefer to work on their own forks.
...
But given that the master branch is protected, only admins are able to do that.

A PR should be reviewed before being merged.
Unless the PR is trivial in which case one is allow to merge it without review.

Reviewer can leave comments on the PR. Some comments are just nitpicks but some other comments should be addressed before merrging the PR.
...
One can edit the commit message to make it nicer and removed the artifacts of all the "fixups"
...
There is a nightly build that run every night.
It will build release artifact and upload them to the `nightly` Github release.
And will also generate the docs, wasm build of examples and make them accessible on snapshots.slint.dev/master

It will also run a few extra tests.

## Release

New minor release of slint are done every couple of months.
...
The process to triage and assign label is documented in the the docs/triage.md file.
